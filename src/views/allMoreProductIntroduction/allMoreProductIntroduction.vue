    ...
    <div>
        ...
        <div class="looking">
            <div class="companyPublicityItem">
                ...
                <div class="position">
                    当前位置：永基首页>产品介绍>更多产品介绍
                </div>
            </div>
        </div>
        <CommonMain placeholder="请输入产品介绍" :list="list" :total="total" @searchTitle="searchTitle" @changePage="changePage"
            ref="commonMainRef" />
        <CommonFooter />
    </div>
</template>
...
import { getProductIntroduction } from '@/api/home'
export default {
    ...
    methods: {
        ...
        getNewsList(pageNum = 1, pageSize = 10, title = '') {
            getProductIntroduction(pageNum, pageSize, title).then((res) => {
