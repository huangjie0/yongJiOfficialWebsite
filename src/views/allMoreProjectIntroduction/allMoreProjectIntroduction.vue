    ...
    <div>
        ...
        <div class="looking">
            <div class="companyPublicityItem">
                ...
                <div class="position">
                    当前位置：永基首页>工程介绍>更多工程介绍
                </div>
            </div>
        </div>
        <CommonHeader title='工程介绍' />
        <CommonMain placeholder="请输入工程介绍" :list="list" :total="total" @searchTitle="searchTitle" @changePage="changePage"
            ref="commonMainRef" />
        <CommonFooter />
    </div>
</template>
...
import { getProjectIntroductionInfo } from '@/api/home'
export default {
    ...
    methods: {
        ...
        getNewsList(pageNum = 1, pageSize = 10, title = '') {
            getProjectIntroductionInfo(pageNum, pageSize, title).then((res) => {
